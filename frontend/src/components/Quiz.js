...
class Quiz extends Component {
    ...
    prepareData = (data) => {
        ...
        for (let input of data) {
            let option = [input.optionA, input.optionB, input.optionC, input.optionD]
            let ans = [input.answerA, input.answerB, input.answerC, input.answerD]
            let correct = []
            let wrong = []
            for (let i=0; i<4; i++) {
                if(ans[i] === true && correct.length === 0){
                    correct.push(option[i])
                } else {
                    wrong.push(option[i])
                }
            }
            question.push({
                question_string: input.question,
                choices: {
                    correct: correct[0],
                    wrong: wrong
                }
    ...
    prepareQuestions = (topic) => {
        axios.get(
            "http://localhost:8080/getQues",
            {
                params: {
            ...
            if (res.status === 200) {
                ...
                let questions = this.prepareData(res.data)
                this.setState({questions: questions, ready: true})
            } else {
                localStorage.removeItem("Username")
                localStorage.removeItem("Token")
                window.location.href = "/login"
    ...
    componentDidMount = () => {
        (function($) {
            $.easyPieChart = function(el, options) {
                ...
                this.init = function() {
                    ...
                    if (typeof window.G_vmlCanvasManager !== "undefined" && window.G_vmlCanvasManager !== null) {
                        window.G_vmlCanvasManager.initElement(_this.canvas);
                    }
        ...
        (function($, get_question, populate) {
            ...
            Quiz.prototype.render = function(container) {
                ...
                $('#submitButton').click(function() {
                    ...
                    $('.score-label h1').html('Hi, ' + 'Your Score<i class="smiley"></i>');
            ...
            $(document).ready(function() {
                ...
                $('#name-form').on('submit', function(event) {
                    ...
                    if (username !== '' && username !== undefined) {
                        $('#name-submit-button').html("Loading.")
                        populate(username)
                        setTimeout( () => {
                            all_questions = get_question()
        ...
        })(jQuery, this.getQuestion, this.prepareQuestions);
    }
